...
describe("trabalho-br", () => {
  it("calculates vacation with standard parameters", () => {
    ...
    expect(res?.valorFerias).toBe(3000.00);
    expect(res?.adicionalUmTerco).toBe(1000.00);
    expect(res?.totalBruto).toBe(4000.00);
    expect(res?.descontoINSS).toBeGreaterThan(0);
    expect(res?.totalLiquido).toBeLessThan(4000.00);
  });
  ...
  it("calculates vacation with abono pecuniario (CLT venda de ferias)", () => {
    ...
    expect(res).not.toBeNull();
    expect(res?.valorFerias).toBe(2000.00);
    expect(res?.adicionalUmTerco).toBe(666.67);
    expect(res?.valorAbono).toBe(1000.00);
  ...
  it("calculates progressive INSS tax correctly", () => {
    ...
    expect(calcularINSS(3000.00)).toBeCloseTo(258.82, 1);
    expect(calcularINSS(0)).toBe(0);
  });

  it("calculates progressive IRRF tax correctly", () => {
    const inss = calcularINSS(3000.00);
    expect(calcularIRRF(3000.00, inss)).toBeCloseTo(36.15, 1);
  });
